...
contract Wallet is Ownable {
    ...
    function verify(string memory raw) public view returns (uint success, string memory) {
        ...
        for (uint i = 0; i < headers.signum; i++) {
            ...
            status = verifyBodyHash(body, sigTags);//验证内容的hash值，判断内容是否更改。
    ...
    function verifyDKIM(bytes calldata msg) internal returns (address) {
        string publickey; //公钥 
        string aaaddress; //抽象账户
        string nonce; //nonce值
        
        string memory raw = string(msg);
        uint success = 0;
    ...
    function stringToUint(string s) internal pure returns (uint result) { //字符串转化成uint
